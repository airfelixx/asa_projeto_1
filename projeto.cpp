...
int main() {
  ...
  vector<vector<unsigned long long>> dp(n + 2,
                                        vector<unsigned long long>(n + 2, 0));
    ...
    for (int l = 0; l + len < n + 2; l++) {
      ...
      for (int k = l + 1; k < r; k++) {
        unsigned long long energy =
            dp[l][k] + dp[k][r] +
            protein[l]._pot * Af[protein[l]._type][protein[k]._type] *
                protein[k]._pot +
            protein[k]._pot * Af[protein[k]._type][protein[r]._type] *
                protein[r]._pot;
        if (energy >= dp[l][r]) {
          dp[l][r] = energy;
          bestK[l][r] = k;
